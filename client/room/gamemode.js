...
RedTeam.Propertues.Get('Deaths').Value <= 1) {
...
function SpawnTeams(Teams) {
 Teams.All.forEach(SpawnTeams => {
   SpawnTeams.Spawn();
          } 
 } 

function SetInventoryPreparation() {
...
function SetInventoryWinners(Winners) {
 const inventory = Inventory.GetContext(Winners);
  inventory.Main.Value = true;
...
function SetInventoryLoosers(Loosers) {
